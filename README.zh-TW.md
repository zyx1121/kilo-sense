...
- **連續逐字稿** — 可拖動的 overlay 視窗累積全文；小模型背景把生稿補標點、修辨識錯字、分段 — 灰字尾巴一直流入，幾秒後被整理過的白字取代，diarizer 認得出講者時自動按人分塊、塊頭標名
...
- **分人** — 只要有語音在播（影片、podcast、通話），本機 diarizer（FluidAudio LS-EEND）就分辨講者；多講者內容的逐字稿自動分塊 — 一般內容標「**講者 A / B**」、會議標「**對方 A / B**」，單講者內容保留更有用的 app・標題來源。背景由 `gpt-5.4-mini` 讀近期輪替把標籤升級成角色與人名 —「**主持人 / 旁白 / 小明**」— 名字必須在逐字稿裡實際出現過才採用（不瞎猜）。名字一旦確認，該聲音就在本機註冊聲紋（`~/.kilo/voices/`），之後跨影片、跨會議、跨重啟直接認人，不再經 LLM
- **會議模式** — 系統音訊裡沒有你自己的聲音，開會時逐字稿會少掉你這側；從選單列打開後 mic 持續錄，你的發言以「**我**」標進逐字稿、系統音訊維持對方那側。請戴耳機（尚無回音消除）
...
```mermaid
flowchart TD
    audio["🔊 系統音訊 — 跟 ASR 吃同一條 16 kHz 流"]
    gate["語音閘<br/>ASR 有動靜就開 · 靜音 30 秒關 · 5 秒環形暫存回灌開頭"]
    diar["🧭 LS-EEND 分人 — 本機,43 MB<br/>有聲紋庫時開機即預熱"]
    tl["講者時間軸<br/>定稿段 + 暫定段,對齊 ASR 時鐘"]
    label["polisher 取批時才解析標籤<br/>講者 / 對方 A·B — 單講者內容保留 app・標題來源"]
    llm["☁️ 歸屬 — gpt-5.4-mini,structured output<br/>角色 + 人名 · 稱呼語邏輯 · 引句驗證<br/>每 60 秒;全員命名後降速到 10 分鐘"]
    enroll["聲紋註冊<br/>同一綁定連續兩輪一致 → enrollSpeaker<br/>音訊取自 120 秒環形暫存"]
    voices[("~/.kilo/voices/*.f32<br/>原始 16 kHz 音訊 — 模型升級也不作廢")]

    audio --> gate --> diar --> tl --> label
    label -->|近期輪替| llm -->|"連續兩輪一致"| enroll --> voices
    voices -. "開機 re-enroll → 認識的聲音直接聲學辨認,不經 LLM" .-> diar
...
三個時序設計撐住準確與省錢。標籤在 polisher 取批時才解析、不在 commit 當下 — 那時 diarizer 的**暫定段**已經蓋得到「長句立即觸發整理」的情況。LLM 歸屬寫死**稱呼語邏輯**（你說出口的名字幾乎都是「對方」的，只有自我介紹綁自己），且名字必須附上驗證得過的逐字稿引句 — 又因為聲紋庫不像顯示名會自癒，註冊額外要求**同一綁定連續兩輪一致**。畫面上的人全認完後，歸屬從一分鐘一次降速到十分鐘一次 — 認識的聲音直接聲學辨認，LLM 已無事可做。刪掉 `~/.kilo/voices/` 即全部遺忘。
